...
- 운영체제는 한 번에 여러 작업을 메모리에 적재한다. 이들 작업은 처음에는 디스크의 작업 풀 내에 유지된다. 오ㅠ냐하면 메모리 내에 동시에 저장할 수 있는 작업의 수는 통상 작업 풀에 둘 수 있는 작업의 수보다 훨씬 적기 때문이다. 작업 풀은 디스ㅡㅋ 내의 모든 프로세스로 구성되며 이들은 주 메모리의 할등을 기다린다.
